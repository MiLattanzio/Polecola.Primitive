...
public class FloatTest
{
    ...
    {
        var start = Random.Shared.NextSingle();
        var end = start + (float.Epsilon * Random.Shared.Next());
        var i = 0ul;
        var current = 0d;
        foreach (var value in Float.Range(start, end))
        {
            Assert.That(value, Is.GreaterThan(current));
            current = value;
            i++;
        }
        var count = Float.Count(start, end);
